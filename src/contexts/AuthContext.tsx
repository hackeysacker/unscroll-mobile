...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function initializeAuth() {
      try {
        ...
        // If no user found, create a guest user to allow app to function
        if (!user && mounted) {
          // This triggers onboarding flow which allows the app to work without sign up
        }
        
        // Force loading to complete after max 5 seconds (backup)
        setTimeout(() => {
          if (mounted) {
            setIsLoading(false);
          }
  ...
  const completeOnboarding = async (goal: GoalType) => {
    // Create a guest user if none exists (for offline mode)
